...
from app.models.study_log import (
    StudyLogCreate,
...
router = APIRouter(
    prefix="/study-logs", tags=["study-logs"], dependencies=[Depends(require_session)]
)
...
def create_study_log(
    payload: StudyLogCreate, service: StudyLogService = Depends(get_study_log_service)
) -> StudyLogOut:
...
def get_study_log(
    study_log_id: str, service: StudyLogService = Depends(get_study_log_service)
) -> StudyLogOut:
